...
const matchReturn = v.object({
  ...
  timestamps: v.object({
    ...
    matchStartedTimestamp: v.optional(v.number()),
  }),
...
async function trackApiUsage(ctx: { db: any }, keyId: Id<"apiKeys">): Promise<void> {
  const now = Date.now();
  const windowStart = now - RATE_LIMIT_WINDOW_MS;
    ...
    return {
      match: {
        ...
        timestamps: {
          ...
          matchStartedTimestamp: selectedMatch.tennisState?.matchStartedTimestamp,
        },
        participant1,
        participant2,
    ...
    if (args.status) {
      tournaments = await ctx.db
        ...
        .withIndex("by_created_by_and_status", (q) =>
          q.eq("createdBy", keyValidation.userId).eq("status", args.status!)
        )
        .collect();
    } else {
      tournaments = await ctx.db
        .query("tournaments")
        .withIndex("by_created_by", (q) => q.eq("createdBy", keyValidation.userId))
        .collect();
    }
