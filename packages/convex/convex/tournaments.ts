...
export const getTournament = query({
  ...
  returns: v.union(
    v.object({
      ...
      defaultMatchDuration: v.optional(v.number()),
      scheduleBuffer: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    ...
    const scorerTournamentDocs = await Promise.all(
      scorerAssignments
        ...
        .map((a) => ctx.db.get("tournaments", a.tournamentId))
    );
...
/**
 * Generate bracket matches for a tournament (or a specific bracket within it).
 * Exported so tournamentBrackets.ts can reuse it.
 */
export async function generateBracketMatches(
  ...
  const format = options?.format ?? tournament.format;
  const bracketId = options?.bracketId;
  ...
  // Process bye matches - auto-advance winners (with recursive chain)
    ...
    if (winnerId) {
      ...
      if (fullMatch.nextMatchId) {
        ...
        if (nextMatch) {
          ...
          // Check if the next match is also a bye (only one participant after all feeders complete)
    ...
    if (brackets.length > 0) {
      ...
      for (const bracket of brackets) {
        ...
        if (bracketParticipants.length >= 2 && bracket.status === "draft") {
          ...
          await ctx.db.patch("tournamentBrackets", bracket._id, { status: "active" });
        }
      }
  ...
  handler: async (ctx, args) => {
    ...
    for (let i = 1; i <= actualSize; i++) {
      ...
      if (assignedParticipantId) {
        ...
        await ctx.db.patch("tournamentParticipants", assignedParticipantId, {
          seed: i,
          isPlaceholder: false,
        });
        const participant = await ctx.db.get("tournamentParticipants", assignedParticipantId);
        if (participant) {
