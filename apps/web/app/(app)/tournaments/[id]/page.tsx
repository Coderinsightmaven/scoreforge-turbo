...
function ScorersTab({
  ...
  const handleDeactivateTempScorer = async (scorerId: string) => {
    if (!confirm("Deactivate this scorer? They will no longer be able to log in.")) return;
    try {
  ...
  const handleResetPin = async (scorerId: string) => {
    if (!confirm("Reset this scorer's PIN? Their current sessions will be invalidated.")) return;
    try {
  ...
  const handleDeleteTempScorer = async (scorerId: string) => {
    if (!confirm("Permanently delete this scorer? This cannot be undone.")) return;
    try {
