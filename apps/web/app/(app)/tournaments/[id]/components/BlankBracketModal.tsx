  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="surface-panel surface-panel-rail w-full max-w-2xl max-h-[90vh] animate-scaleIn flex flex-col">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <p className="text-sm text-muted-foreground mb-4">
            ...
            {realParticipants.length > 0 &&
              " Assign existing participants to seeds or leave slots empty to fill in later."}
          </p>

          {/* Bracket Size Selector */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-foreground mb-2">Bracket Size</label>
            <select
              value={bracketSize}
              onChange={(e) => {
              ...
              className="input"
            >
              <option value={4} disabled={realParticipants.length > 4}>
                4 participants
              </option>
              <option value={8} disabled={realParticipants.length > 8}>
                8 participants
              </option>
              <option value={16} disabled={realParticipants.length > 16}>
                16 participants
              </option>
              <option value={32} disabled={realParticipants.length > 32}>
                32 participants
              </option>
              <option value={64} disabled={realParticipants.length > 64}>
                64 participants
              </option>
            </select>
          </div>
            ...
            <div className="mb-6">
              ...
              {unassignedParticipants.length > 0 && (
                <div className="mb-4">
                  <p className="text-xs text-muted-foreground mb-2">
                    Unassigned - click a slot below to assign:
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {unassignedParticipants.map((p) => (
                      <div
                        key={p._id}
                        className="rounded-xl border border-brand/30 bg-brand/10 px-3 py-1.5 text-sm text-brand"
                      >
                        {p.displayName}
                      </div>
                    ))}
              ...
              {seedAssignments.length > 0 && (
                <p className="text-xs text-success mb-2">
                  {seedAssignments.length} participant{seedAssignments.length !== 1 ? "s" : ""}{" "}
                  assigned to seeds
                </p>
              )}
            </div>
          ...
          {/* Seed Slots */}
          <div>
            <h4 className="text-sm font-medium text-foreground mb-3">Bracket Slots</h4>
                    ...
                    {assigned ? (
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          className="text-xs text-error hover:text-red-dim flex-shrink-0"
                          title="Remove"
                        >
                          X
                        </button>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border/70 p-6 flex-shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={handleGenerate} disabled={generatingBlank} variant="brand" size="sm">
