    ...
    <View className="flex-1 bg-white">
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-1">
            <View className="mb-1 flex-row items-center">
              <View className="mr-2 rounded bg-amber-500 px-2 py-0.5">
                <Text className="text-xs font-bold text-white">TEMP SCORER</Text>
              </View>
            </View>
            <Text className="text-lg font-bold text-gray-900">{session.displayName}</Text>
      ...
      {matches === undefined ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const isReady =
              item.participant1 && item.participant2 && item.status !== 'completed';
            return (
              <TouchableOpacity
                ...
                {isReady && item.status !== 'live' && (
                  <View className="mt-3 items-center">
                    <View className="rounded-full bg-amber-100 px-4 py-1">
        ...
        {session.expiresAt && session.expiresAt - Date.now() < 2 * 60 * 60 * 1000 && (
          <View className="bg-amber-50 px-4 py-2">
            <Text className="text-center text-xs text-amber-700">
              Session expires in{' '}
              {Math.round((session.expiresAt - Date.now()) / (60 * 60 * 1000))} hours
            </Text>
          </View>
        )}
      </SafeAreaView>
